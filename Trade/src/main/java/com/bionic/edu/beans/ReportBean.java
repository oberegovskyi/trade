...
public class ReportBean implements Serializable {
	...
	private void createBarModel() {
		...
		barModel.setTitle("Звіт по "+tempFish.getFishName()+" за період від "+ dateFormat.format(tempDate1)+" до "+ dateFormat.format(tempDate2));
	...
	public void getReport() {
		...
		if (!dd.isEmpty()) {
			...
			tempDate1 = null;
			tempDate2 = null;
			RequestContext.getCurrentInstance().scrollTo("reportWeightForm:chart");
		} else {
			FacesContext context = FacesContext.getCurrentInstance();
			context.addMessage(":reportWeightForm:repGrowl", new FacesMessage("Увага", tempFish.getFishName()+ " не продавалась за період з "+dateFormat.format(tempDate1)+ 
					 " до "+dateFormat.format(tempDate2)));
		}
	...
	public void dateChange1(SelectEvent event) {
		...
		FacesContext context = FacesContext.getCurrentInstance();
		context.addMessage(":reportWeightForm:repGrowl", new FacesMessage("Увага","Початкова дата встановлена "+dateFormat.format(date)));
	}
	...
	public void dateChange2(SelectEvent event) {
		...
		java.util.Date date = (java.util.Date)event.getObject();
		FacesContext context = FacesContext.getCurrentInstance();
		context.addMessage(":reportWeightForm:repGrowl", new FacesMessage("Увага","кінцева дата встановлена "+dateFormat.format(date)));
	}
